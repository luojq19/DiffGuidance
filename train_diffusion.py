...
def save_data_pt(dataset, save_path):
    all_data = []
    for ori_data in tqdm(dataset):
        all_data.append(ori_data.__dict__['_store'].__dict__['_mapping'])
    print(f'saving {len(all_data)} entries to {save_path}')
...
if __name__ == '__main__':
    ...
    # print(train_set[0])
    # print('asld')
    print(type(train_set[0].__dict__['_store'].__dict__['_mapping']))
    # print(train_set[0].protein_atom_feature)
    # print(train_set[0].protein_atom_feature.shape)
    # print(len(train_set))
    save_data_pt(train_set, '/work/jiaqi/Diff2D3D/data/crsd_train.pt')
    save_data_pt(val_set, '/work/jiaqi/Diff2D3D/data/crsd_val.pt')
    input()
    ...
    def train(it):
        ...
        for _ in range(config.train.n_acc_batch):
            ...
            results = model.get_diffusion_loss(
                ...
                batch_smiles=batch.ligand_smiles
            )
            # print(results)
            # input()
            loss, loss_pos, loss_v, loss_guide = results['loss'], results['loss_pos'], results['loss_v'], results['loss_guide']
        ...
        if it % args.train_report_iter == 0:
            logger.info(
                '[Train] Iter %d | Loss %.6f (pos %.6f | v %.6f | guide %.6f) | Lr: %.6f | Grad Norm: %.6f' % (
                    it, loss, loss_pos, loss_v, loss_guide, optimizer.param_groups[0]['lr'], orig_grad_norm
                )
            )
    ...
    def validate(it):
        # fix time steps
        sum_loss, sum_loss_pos, sum_loss_v, sum_loss_guide, sum_n = 0, 0, 0, 0, 0
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(val_loader, desc='Validate'):
                ...
                for t in np.linspace(0, model.num_timesteps - 1, 10).astype(int):
                    ...
                    sum_loss_guide += float(loss_guide) * batch_size
                    sum_n += batch_size
        ...
        avg_loss_guide = sum_loss_guide / sum_n
        ...
        logger.info(
            '[Validate] Iter %05d | Loss %.6f | Loss pos %.6f | Loss v %.6f e-3 | Loss guide %.6f | Avg atom auroc %.6f' % (
                it, avg_loss, avg_loss_pos, avg_loss_v * 1000, avg_loss_guide, atom_auroc
            )
        )
